...
import PatientLabDetail from "@/components/laboratory/PatientLabDetail";
import EmptyState from "@/components/common/EmptyState/EmptyState";
import { cn } from "@/lib/utils";

export default function LabPage() {
  const { data: allLabs = [], isLoading } = useAllLabs();
  ...
  const patientMap = new Map<string, { patientId: string; ptName: string; ptOpd?: string; labs: typeof allLabs }>();
  allLabs.forEach((l) => {
    if (!patientMap.has(l.patientId)) {
      patientMap.set(l.patientId, {
        patientId: l.patientId,
        ptName: l.ptName || "Unknown Patient",
        ptOpd: l.ptOpd,
        labs: [],
      });
    }
    patientMap.get(l.patientId)!.labs.push(l);
  });

  const patientLabs = Array.from(patientMap.values()).filter((g) => {
    if (!q) return true;
    return (
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-medium text-ink-3">Loading laboratory investigations…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fade-in p-6 sm:p-8 pb-20 max-w-7xl mx-auto">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-2">
        <div>
          <h1 className="font-serif text-3xl font-medium tracking-tight text-ink">Laboratory</h1>
          <p className="text-sm font-medium text-ink-3">
            {pendingCount} pending · {completedCount} completed · {allLabs.length} total
          </p>
        </div>
        <button
      ...
      {patientLabs.length === 0 ? (
        <EmptyState
          ...
          subtitle="Request a lab investigation using the button above."
        />
      ) : (
        <div className="space-y-3">
          {patientLabs.map(({ patientId, ptName, labs }) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs font-semibold text-ink-3">
                        {labs.length} test{labs.length !== 1 ? "s" : ""} · {pending} pending
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {pending > 0 && (
                      <span className="rounded-full bg-status-elevated-bg border border-amber-200 px-3 py-1 text-xs font-bold uppercase text-status-elevated">
                        {pending} pending
                      </span>
                    )}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  {labs.length > 4 && (
                    <span className="text-xs font-bold text-ink-3 self-center">+{labs.length - 4} more</span>
                  )}
                </div>
              </div>
            );
