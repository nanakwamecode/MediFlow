...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Brand */}
      <div className="flex h-[62px] shrink-0 items-center justify-between border-b border-border/60">
        <div
          className={cn(
        ...
        >
          ...
          <div className="flex flex-col">
            <h1 className="font-serif text-[1.45rem] font-medium text-ink tracking-wide leading-none mt-0.5">
              MediFlow
            </h1>
      ...
      <div className={cn("shrink-0 overflow-hidden border-t border-border/60 transition-all", sidebarCollapsed ? "p-2" : "p-3 px-3")}>
        <button
          ...
          className={cn(
            "flex w-full cursor-pointer items-center gap-3 overflow-hidden rounded-xl border border-border/50 bg-bg/50 text-left transition-all hover:bg-bg hover:border-border",
            sidebarCollapsed ? "justify-center p-0 h-[36px] w-[36px]" : "p-2 mb-1.5"
          )}
          title="View Profile"
        >
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent/10 font-serif text-[0.8rem] font-bold text-accent shadow-inner">
            {user?.displayName ? user.displayName.replace('Dr. ', '').split(' ').map(n=>n[0]).join('').slice(0,2).toUpperCase() : "DR"}
          </div>
          {!sidebarCollapsed && (
            <div className="flex-1 overflow-hidden">
              <div className="truncate text-[0.8rem] font-semibold text-ink tracking-wide">
                {user?.displayName ?? "Doctor"}
              </div>
              <div className="flex items-center gap-1.5 text-[0.6rem] font-mono text-ink-3">
